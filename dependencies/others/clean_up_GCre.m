function clean_up_GCre(sitepath)
combinepath=fullfile(sitepath,'combined');
cd(combinepath);
load('p_data_detail.mat');
repath=fullfile(sitepath,'results/newGC');
cd(repath)

    % 载入totalGC.mat文件
    if ~isfile('totallGC.mat')
        return
    end
    matData = load('totallGC.mat');
    
    % 获取totalGC结构体中的字段名
    fields_s2p = fieldnames(matData.totalGCs2p);
    fields_p2s = fieldnames(matData.totalGCp2s);
    ...
    for i = 1:length(fileList)
        ...
        if ~ismember(spkc_part, increaseName)
            ...
            if isfield(matData.totalGCs2p, [lfp_part '_' spkc_part])
                matData.totalGCs2p = rmfield(matData.totalGCs2p, [lfp_part '_' spkc_part]);
            end
            if isfield(matData.totalGCp2s, [lfp_part '_' spkc_part])
                matData.totalGCp2s = rmfield(matData.totalGCp2s, [lfp_part '_' spkc_part]);
            end
        end
    ...
    % 保存修改后的totalGC.mat文件
    save('totallGC.mat', '-struct', 'matData');
end
