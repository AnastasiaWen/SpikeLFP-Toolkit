function maxnum=oicon_sp_neuro_tune(sitepath)
...
diPath = fullfile(sitepath,'results\orientationC_tun');
% rmdir(diPath,"s")
mkdir(diPath);
...
% uniqAngle=[45,90,135,180];% 0,45,90,135,180 per 24times
    ...
    if startsWith(varName, 'SPKC')
        ...
        for i=1:4
            if i~=1
                ...
                rc=find(totalangle==uniqAngle(1)|totalangle==uniqAngle(5));
            end
                refr=[];
                if ~isempty(rc)
                    for k=1:length(rc)
