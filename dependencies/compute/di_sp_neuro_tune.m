function maxnum=di_sp_neuro_tune(sitepath)
%计算di值，并返回统计
...
for h = 1:length(varNames)
    ...
    if startsWith(varName, 'SPKC')
        ...
        sin_component = sum(totalfr .* sin(deg2rad(uniqAngle)));
        cos_component = sum(totalfr .* cos(deg2rad(uniqAngle)));
        ...
        for p = 1:num_permutations
            ...
            sin_component_perm = sum(totalfr .* sin(deg2rad(perm_angle)));
            cos_component_perm = sum(totalfr .* cos( deg2rad(perm_angle)));
