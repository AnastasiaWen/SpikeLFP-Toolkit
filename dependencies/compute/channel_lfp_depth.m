function channel_lfp_depth(sitepath)
% close all;
% combinepath=fullfile(sitepath,'combined');
% cd(combinepath);
% load Combined.mat
% 
% savePath = fullfile(sitepath,'results\channel_lfp_depth');
% mkdir(savePath);
% %%
...
%%
close all;
load(fullfile(sitepath,'results/channel_lfp/ProbeLFP.mat'));
...
% 设置尺度
time_scale = 0.2; % ms
amplitude_scale = 2; % μV


vars_probelfp = who('-regexp', '^Probe');
probe_name = who('-regexp', '^Probe');

colors=parula(numel(vars_probelfp));
figure;
set(gcf,'unit','normalized','position',[0,0,0.25,1]);
mean0=0;
hold on;
for p=1:numel(vars_probelfp)
    eval(['lfp=',vars_probelfp{p},'.totalBand;']);
    
    plot(lfp.t-1,(10*log10(sum(lfp.S,1)))+(p-1)*amplitude_scale-(min(10*log10(sum(lfp.S,1)))-0),'LineWidth', 2,'Color',colors(p,:));
    %mean0=max(sum(lfp.S,1));
end
