function GC_Other_channel(sitepath)
%granger causality计算不同channel的spk和lfp结果
...
savePath = fullfile(sitepath,'results\newGC');
...
for i=1:numel(vars_lfp)
    ...
    for ss=1:numel(vars_sp)
        ...
        % end
        [~,~,~,~,tt,f,GCs2p,GCp2s] = spk_lfp_GC(tmplfp, tmpspk, params, movingwin);
        eval(['totalGCs2p.',vars_lfp{i},'_',vars_sp{ss},'=GCs2p;']);
        eval(['totalGCp2s.',vars_lfp{i},'_',vars_sp{ss},'=GCp2s;']);
...
savename={'tt','f','totalGCp2s','totalGCs2p','pGCs2p','pGCp2s'};
